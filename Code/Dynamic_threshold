...
exports.init = function(Chosen_Index, Chosen_Geometry, Chosen_Geometry_M, Chosen_Geometry_H, adv, newcol3, CleanCollection, range3, counter4, counter3) {

  var Otsucol = exports.Otsu(range3, adv, newcol3, CleanCollection, Chosen_Geometry, Chosen_Geometry_H, counter3, Chosen_Index);
...
exports.Otsu = function(range3, adv, newcol3, CleanCollection, Chosen_Geometry, Chosen_Geometry_H, counter3, chosenIndex) 
{
  var thresholdOtsu = function(count, trlist)
  {
      ...
      var date = ee.Date(range4.get(ee.Number(count).multiply(adv)));
      
      // select the imagecollection for output
      var TresholdImage = newcol3.filterDate(date, date.advance(adv,'month'));
      
      // convert to Image
      var TreshImage = ee.Image(TresholdImage.mean());
      
      // select the imagecollection for processing
      var TresholdImageAll = CleanCollection.filterDate(date, date.advance(adv,'month'));
